$(function(){
  ...
  let companyHeaders = 'id,company_name,contact_name,phone,date_added';
  ...
  let initialQuery = `SELECT ${companyHeaders} FROM ${initialTable}`;
...
function sqlQueryModal(initialTable, id) {
  let query = `SELECT * FROM ${initialTable} WHERE id='${id}'`;
...
//       let tbl = table.Tables_in_clientdatafile;
//       let queryFields = `SHOW COLUMNS from ${tbl}`;
//       db.connect().query(queryFields, function (err, result) {
//         if (err) throw err;
//         table['fields'] = result;
//       });
//       // getting rows from each table
//       let queryTables = `SELECT * from ${tbl}`;
...
//   });
//
//   // dynamically adding initial tableData limiting with maxHeaders let
//   function addTableData(tableNum) {
//     let data = tableData.tables[tableNum].data;
//     let str = '';
...
//   }
//
//   // dynamically adding headers from tableData limiting with maxHeaders let
...
//       let tableName = table.Tables_in_clientdatafile;
//       str += `
...
// let createTblSql = `IF NOT EXISTS (SELECT * FROM dbo.sysobjects where id = object_id(N'dbo.[my_table_name]') and OBJECTPROPERTY(id, N'IsTable') = 1)
// 	BEGIN
